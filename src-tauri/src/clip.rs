...
impl AudioClipPlayer {
    pub fn new(clip: Arc<AudioClip>, output_sample_rate: u32, volume: f32) -> Self {
...
fn load_wav_mono(path: &Path) -> Result<AudioClip, Box<dyn Error>> {
    ...
    let samples = match spec.sample_format {
        ...
        hound::SampleFormat::Int => {
            ...
            reader
                .samples::<i32>()
                .map(|sample| sample.map(|sample| (sample as f32 / max_amplitude).clamp(-1.0, 1.0)))
                .collect::<Result<Vec<_>, _>>()?
        }
    };
...
fn sample_interpolated(samples: &[f32], position: f32) -> Option<f32> {
    ...
    Some(samples[index] + (samples[index + 1] - samples[index]) * fraction)
}
